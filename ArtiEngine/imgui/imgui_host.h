...
// 宿主侧的 ImGui：context、SDL3 平台后端、字体图集。
//
// 放在 ArtiEngine 而不是 ArtiRenderer：ImGui 的平台后端要链 SDL，而 ArtiRenderer 刻意不链
// Platform/SDL（它只认 FrameOverlay 里的一个 ImDrawData 指针）。引擎层本来就要管窗口，
// 所以这层才是它的归属。ArtiRenderer 的 samples 下还留着一份，用于它自己的 ImGui pass 冒烟测试。
//
// 单独一个 target（ArtiEngine::ImGui）而不是并进 artiengine：核心库不该因为「可能要画 UI」
// 就把 SDL 和 imgui 拖进每个下游。
//
