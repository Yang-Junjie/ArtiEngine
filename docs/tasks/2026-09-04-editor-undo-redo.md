...
**当前进度：只剩 4.2 那一条人工验收。** 代码、文档、自动化测试全部完成并通过。

阶段 3.4 的七条**用户 2026-09-04 在编辑器里全走过，没有问题**：一次拖拽一条历史项、点了不改
东西不产生历史项、建 / 删 / 复制 / 拖资产都能撤销和重做、删除撤销后选中回来、模拟中灰化、
换场景清历史、拖拽中按 Ctrl+Z 不出事。**这七条是这个任务真正的防线，它们过了就说明
「粗糙的提交信号 + 文本比较」这套设计在真实交互下是成立的。**

下一步：4.2 那条 Environment 的验收（一个 20 秒的检查，见那一步）。过了就把头部状态改成
「已完成」。

`ctest` **10 条全绿**（新增 `scene_snapshot_smoke` 和 `edit_history_test`）。编辑器和
`arti_player` 都起得来、渲得出、日志里没有新的 error。
...
- [~] **4.2 修 Environment 的 UUID 缓存**（D9、证据 6） ← 进行中：**代码好了，就差这一条验收**
  ...
  - **代码已完成**，比计划多了一个 `initialized` 位。不能光比较 `applied`：它的初值是 `UUID{0}`，
    而「没设贴图」的组件里存的正好也是 0，第一帧不会同步，输入框会是空的而不是一串 0
    （那是行为变化，不是这个 bug 要修的东西）。
  - **3.4 那七条覆盖不到这一条**：它只在「改了 Equirect Texture 这个字段」时才发作，
    而且只有那个 Environment 实体正选中着的时候。
...
- [ ] **5.2 任务收尾** ← 进行中：等 3.4 和 4.2 的人工验收过了再把头部状态改成「已完成」
...
6. 阶段 4.2 那条 Environment 的验收过（修之前是红的）。**还没做** —— 3.4 覆盖不到它。
