...
**当前进度：未开工。下一步 = 阶段 1（`Frustum` 类型 + 单元测试）。**

开工前已经核实完的事实都写进「背景与现状」了，包括一条**实测**结论：
`ARTIRENDERER_BUILD_TESTS=ON` 之后那三个现存测试全过（见 D2 的证据），所以 D2 是安全的。
...
证据 7 已经确认全工程用 `perspectiveRH_ZO` / `orthoRH_ZO`。写成 NO 的公式会让 near 面
偏到相机后面 —— **表现是「几乎什么都不剔」，也就是一个看起来能跑、实际没生效的实现**。
所以阶段 1 的验收必须有一条「相机背后的盒子被剔掉」的断言。
...
3. `frustum_test` 的「相机背后」那条断言**反向验证过会失败**（把 near 面改成 NO 公式）。
...
第一种表现为 `culled` 几乎恒为 0，第二种表现为某些影子消失（而且只在特定相机角度下）。
