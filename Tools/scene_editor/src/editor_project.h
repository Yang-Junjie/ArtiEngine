...
// 编辑器打开的项目。持有 AssetPipeline 和 GpuAssetCache —— 两者的生命周期都绑在项目上：
...
public:
    ...
    tools::asset::AssetPipeline& assetPipeline() noexcept { return m_asset_pipeline; }
    const tools::asset::AssetPipeline& assetPipeline() const noexcept { return m_asset_pipeline; }
    arti::asset::AssetManager& assets() { return m_asset_pipeline.manager(); }
...
private:
    // 打开成功后的公共收尾：打开资产工作区并创建 GPU cache。
    bool finishOpen();

    rendering::Renderer* m_renderer{ nullptr };
    tools::asset::AssetPipeline m_asset_pipeline;
